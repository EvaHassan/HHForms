...
protocol ProfileItemRow: DoubleTextFieldTypeRow {
    var imageItem: ImageFormItem {get set}
}
...
/*extension RowDescriptorFactory {
    static func makeProfileRowDescriptor() -> RowDescriptor {
        let item = FormRowFactory.makeProfileRow()
        return RowDescriptor(formRow: item, rowType: .profileRowType)
    }
}  */
